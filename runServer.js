...
io.on('connection', function(client){
	checkForWake();
	utils.logToFile('logs\\connections.txt', new Date() + " : "+ client.handshake.address + ' connected\r\n');
	clientCount++;
	...
	client.on('disconnect', function() {
		...
		clientCount--;
		utils.logToFile('logs\\connections.txt', new Date() + " : "+ client.handshake.address + ' disconnected\r\n');
		checkForSleep();
  	});
...
function checkForWake(){
	if(serverSleeping){
		...
		utils.logToFile('logs\\connections.txt',new Date() + " : Server wakeup\r\n");
...
function checkForSleep(){
	...
	if(clientCount == 0){
		...
		utils.logToFile('logs\\connections.txt',new Date() + " : Server sleep\r\n");
	}
}
